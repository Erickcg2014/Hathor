...
public class ControllerHato {
    ...
    @GetMapping("/opciones")
    public ResponseEntity<OpcionesHatoDTO> getOpciones() {
        return ResponseEntity.ok(serviceHato.getOpciones());
    }

    // =========== ACTUALIZAR INFO GENERAL =========
    @PutMapping("/{idHato}")
    public ResponseEntity<?> actualizarInfoHato(
        @AuthenticationPrincipal Jwt jwt,
        @PathVariable UUID idHato,
        @RequestBody RegistroHatoDTO dto
    ) {
        try {
            String email = jwt.getClaim("email");
            return ResponseEntity.ok(serviceHato.actualizarInfoHato(idHato, dto, email));
        } catch (RuntimeException e) {
            if ("HATO_NO_ENCONTRADO".equals(e.getMessage()))
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(Map.of("error", "Hato no encontrado"));
